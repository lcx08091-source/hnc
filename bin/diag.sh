...
# v3.8.2 alpha-0: PATH 健壮性,见 service.sh
[ -z "$HNC_SKIP_PATH_HARDENING" ] && [ -z "$HNC_TEST_MODE" ] && export PATH=/system/bin:/system/xbin:/vendor/bin:$PATH
# diag.sh — HNC v3.8.2 自检脚本
#
# 用法:
...
# ── [1/14] HNC 安装目录 ──────────────────────────────────
# v3.8.2 修复:module.prop 在 Magisk/KernelSU 模块目录,
...
# ── [13/14] hotspotd daemon 状态(v3.8.2 重写) ───────────
...
# ── [14/14] dumpsys network_stack 格式探针(v3.8.2) ───────
...
if [ $JSON_MODE -eq 1 ]; then
    # JSON 输出
    printf '{"version":"v3.8.2","pass":%d,"warn":%d,"fail":%d,"checks":[' "$PASS" "$WARN" "$FAIL"
    FIRST=1
